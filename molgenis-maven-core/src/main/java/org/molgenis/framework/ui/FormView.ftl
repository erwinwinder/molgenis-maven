...
<div class="form_header" id="${screen.getName()}">
	...
	<#list screen.getFilters() as filter>
				<label><#if filter_index=0>where: <#else></#if>${filter}</label><img height="16" class="navigation_button" src="res/img/cancel.png" alt="Cancel" onclick="setInput('${screen.name}_form','_self','','${screen.name}','filter_remove','iframe'); document.forms.${screen.name}_form.filter_id.value='${filter_index}'; document.forms.${screen.name}_form.submit();" title="remove filter"/>
	</#list>
			</td>
        -->
		</tr>
	...
	<#list screen.getMenus() as menu>
							<td class="menuitem" id="${screen.name}_menu_${menu.name}" onclick="mopen('${screen.name}_menu_${menu.name}Sub');">
								${menu.label}
								<img src="res/img/pulldown.gif" alt="pulldown"><br />
		...
		<#list menu.getCommands() as command>
			...
			<td width="34%" class="form_title">
				<img class="navigation_button" id="first_${screen.name}" src="res/img/first.png" onclick="setInput('${screen.name}_form','_self','','${screen.name}','first','iframe'); document.forms.${screen.name}_form.submit();" title="go to first record"/>
				<img class="navigation_button" id="prev_${screen.name}" src="res/img/prev.png" onclick="setInput('${screen.name}_form','_self','','${screen.name}','prev','iframe'); document.forms.${screen.name}_form.submit();" title="go to previous record"/>
	...
	<#else>
				...
				<img class="navigation_button" id="next_${screen.name}" src="res/img/next.png" onclick="setInput('${screen.name}_form','_self','','${screen.name}','next','iframe'); document.forms.${screen.name}_form.submit()" title="go to next record"/>
				<img class="navigation_button" id="last_${screen.name}" src="res/img/last.png" onclick="setInput('${screen.name}_form','_self','','${screen.name}','last','iframe'); document.forms.${screen.name}_form.submit()" title="go to last record"/>	
			</td>	
			<td width="33%">
...
<div align="right">
		...
		<img class="navigation_button" src="res/img/filter.png" alt="Add filter" onclick="setInput('${screen.name}_form','_self','','${screen.name}','filter_add','iframe'); document.forms.${screen.name}_form.submit();"/>		
</div>
</#macro>
...
<body>
	...
	<p align="right">
		<img class="edit_button" src="res/img/save.png" alt="Save" onclick="if( validateForm(${screen.name}_new,new Array(${required})) ) { if( window.opener.name == '' ){ window.opener.name = 'molgenis'+Math.random();} document.forms.${screen.name}_new.target = window.opener.name; document.forms.${screen.name}_new.submit(); window.close();}" title="save new record"/>
		<img class="edit_button" src="res/img/cancel.png" alt="Cancel" onclick="window.close();" title="cancel adding a new record"/>
	</p>
</body>
</#macro>
<#--
...
<body>
	...
	<#list screen.getNewRecordForm() as input>
			...
			<tr>
				<td></td>
				<td >					
					<img class="edit_button" src="res/img/save.png" alt="Add filter" onclick=" document.forms.${screen.name}_filter.submit(); window.opener.location.href = window.opener.location.href; window.close();"/>
					<img class="edit_button" src="res/img/cancel.png" alt="Cancel" onclick="window.close();" title="cancel adding a new filter"/>				
				</td>
			</tr>
		</table>
	</form>
...
<body onload="<#list screen.getNewRecordForm().inputs as input><#if !input.isHidden()>${screen.name}_massupdate.${input.name}.disabled = true; </#if></#list>">
	...
	<p align="right">
		<!--<img class="edit_button" src="res/img/save.png" alt="Save" onclick="document.forms.${screen.name}_massupdate.submit(); window.opener.location.href = window.opener.location.href; window.close();" title="update records"/>-->
		<img class="edit_button" src="res/img/save.png" alt="Save" onclick="document.forms.${screen.name}_massupdate.target = window.opener.name; document.forms.${screen.name}_massupdate.submit(); window.close();" title="update records"/>
		<img class="edit_button" src="res/img/cancel.png" alt="Cancel" onclick="window.close();" title="cancel updating records"/>
	</p>
	
	<#else>
	You did not check any records.
	<p align="right">
		<input type="image" src="res/img/cancel.png" alt="Cancel" name="action_cancel" value="Cancel" onclick="window.close();" title="cancel adding a new record"/>
	</p>
	</#if>
</body>
</#macro>
...
<#if readonly != "true">
<img class="edit_button" id="save_${screen.name}" src="res/img/save.png" alt="Save" onclick="if ($('#${screen.name}_form').valid() && validateForm(document.forms.${screen.name}_form,new Array(${required}))) {setInput('${screen.name}_form','_self','','${screen.name}','update','iframe'); document.forms.${screen.name}_form.submit();}" title="save the changes" />
<#-->image class="edit_button" src="res/img/save.png" alt="Save" onclick="setInput('${screen.name}_form','_self','','${screen.name}','update','iframe'); document.forms.${screen.name}_form.submit();}" title="save the changes" /-->
<img class="edit_button" id="reset_${screen.name}" src="res/img/reset.png" alt="Reset" onClick="setInput('${screen.name}_form','_self','','${screen.name}','listview','iframe'); document.forms.${screen.name}_form.submit();" title="stop editing and go to list view" />
<img class="edit_button" id="delete_${screen.name}" src="res/img/delete.png" alt="Delete" onclick="if (confirm('You are about to delete a record. If you click [yes] you won\'t be able to undo this operation.')) { setInput('${screen.name}_form','_self','','${screen.name}','remove','iframe'); document.forms.${screen.name}_form.submit(); }" title="delete current record" />
</#if>
</td></tr></table>		
	</#list>
<br />
	...
	<tr>
		...
		<th><label>&nbsp;</label></th>
			...
			<#list record.inputs as input>				
				...
				<#else>
					<#if input.isHidden()>
			<#--<img src="res/img/open.png" title="show ${input.getLabel()}" onclick="setInput('${screen.name}_form','_self','','${screen.name}','showColumn','iframe'); document.forms.${screen.name}_form.attribute.value ='${input.getName()}'; document.forms.${screen.name}_form.submit();" />-->
					<#else>
...
<tr class="form_listrow${rowcolor}">
	<td>
		<label>${offset}. 
		<#if readonly == "*" >
			<img class="edit_button" src="res/img/recordview.png" title="view record" alt="edit${offset}" onClick="setInput('${screen.name}_form','_self','','${screen.name}','editview','iframe'); document.forms.${screen.name}_form.__offset.value='${offset?string.computer}'; document.forms.${screen.name}_form.submit();">${readonly}</label>
		<#else>
			<img class="edit_button" src="res/img/editview.gif" title="edit record" alt="edit${offset}" onClick="setInput('${screen.name}_form','_self','','${screen.name}','editview','iframe'); document.forms.${screen.name}_form.__offset.value='${offset?string.computer}'; document.forms.${screen.name}_form.submit();">${readonly}</label>
		</#if>
	</td>
...
<body>
	...
	<form action="" method="post" enctype="multipart/form-data" name="${screen.name}_upload">
		...
		<!--<img class="edit_button" src="res/img/save.png" alt="Save" onclick="document.forms.${screen.name}_upload.submit(); window.opener.location.href = window.opener.location.href; window.close();" title="upload csv"/>-->
		<img class="edit_button" src="res/img/save.png" alt="Save" onclick="if( window.opener.name == '' ){ window.opener.name = 'molgenis'+Math.random();} document.forms.${screen.name}_upload.target = window.opener.name; document.forms.${screen.name}_upload.submit(); window.close();" title="upload csv"/>
		<img class="edit_button" src="res/img/cancel.png" alt="Cancel" onclick="window.close();" title="cancel upload csv"/>
		</td></tr>
		</table>
	</form>
</body>
...
<#else>
	...
	<div class="formscreen" id="${screen.getName()}_screen">
		...
		<form name="${screen.name}_form"  id="${screen.name}_form" target="#${screen.name}" action="" method="post" style="margin: 0px;" enctype="multipart/form-data">
			...
			<@form_toolbar screen/>
			<!--<td class="form_collapse" style="width: 12px;float:right;clear:right; margin:5px;"><img src="res/img/minus.png" id="${screen.name}_toggleImage" onclick="javascript:toggleForm('${screen.name}')"/></td>-->
			...
			<#if screen.getFilters()?size gt 0>
				<div style="float: right;font-size-adjust: 0.5;font-style:italic;background:#C0C0C0;border: .2em dotted #000;padding:4px;">
					...
					<#list screen.getFilters() as filter>			
						<b>${filter}</b> <img id="remove_filter_${filter_index}" height="16" class="navigation_button" src="res/img/cancel.png" alt="Cancel" onclick="setInput('${screen.name}_form','_self','','${screen.name}','filter_remove','iframe'); document.forms.${screen.name}_form.filter_id.value='${filter_index}'; document.forms.${screen.name}_form.submit();" title="remove filter"/>
