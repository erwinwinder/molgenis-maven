...
/**
 * Reads StudyDetails_OtherCitations from Excel file.
 */
public class StudyDetails_OtherCitationsExcelReader
{
	private static final Logger logger = Logger.getLogger(StudyDetails_OtherCitationsExcelReader.class);
			
	/**
	 * Imports StudyDetails_OtherCitations from a workbook sheet.
	 */
	...
	{
		File tmpStudyDetails_OtherCitations = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpStudyDetails_OtherCitations.txt");
		if(tmpStudyDetails_OtherCitations.exists()){
			boolean deleteSuccess = tmpStudyDetails_OtherCitations.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpStudyDetails_OtherCitations.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpStudyDetails_OtherCitations.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpStudyDetails_OtherCitations.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpStudyDetails_OtherCitations);
		if(fileHasHeaders){
			int count = new StudyDetails_OtherCitationsCsvReader().importCsv(db, tmpStudyDetails_OtherCitations, defaults, dbAction, missingValue);
			tmpStudyDetails_OtherCitations.delete();
			return count;
		}else{
			tmpStudyDetails_OtherCitations.delete();
			return 0;
