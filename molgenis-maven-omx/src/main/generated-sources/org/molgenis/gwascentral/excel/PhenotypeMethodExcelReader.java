...
/**
 * Reads PhenotypeMethod from Excel file.
 */
public class PhenotypeMethodExcelReader
{
	private static final Logger logger = Logger.getLogger(PhenotypeMethodExcelReader.class);
			
	/**
	 * Imports PhenotypeMethod from a workbook sheet.
	 */
	...
	{
		File tmpPhenotypeMethod = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpPhenotypeMethod.txt");
		if(tmpPhenotypeMethod.exists()){
			boolean deleteSuccess = tmpPhenotypeMethod.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpPhenotypeMethod.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpPhenotypeMethod.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpPhenotypeMethod.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpPhenotypeMethod);
		if(fileHasHeaders){
			int count = new PhenotypeMethodCsvReader().importCsv(db, tmpPhenotypeMethod, defaults, dbAction, missingValue);
			tmpPhenotypeMethod.delete();
			return count;
		}else{
			tmpPhenotypeMethod.delete();
			return 0;
