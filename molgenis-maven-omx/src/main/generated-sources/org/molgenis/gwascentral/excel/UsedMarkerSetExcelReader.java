...
/**
 * Reads UsedMarkerSet from Excel file.
 */
public class UsedMarkerSetExcelReader
{
	private static final Logger logger = Logger.getLogger(UsedMarkerSetExcelReader.class);
			
	/**
	 * Imports UsedMarkerSet from a workbook sheet.
	 */
	...
	{
		File tmpUsedMarkerSet = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpUsedMarkerSet.txt");
		if(tmpUsedMarkerSet.exists()){
			boolean deleteSuccess = tmpUsedMarkerSet.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpUsedMarkerSet.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpUsedMarkerSet.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpUsedMarkerSet.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpUsedMarkerSet);
		if(fileHasHeaders){
			int count = new UsedMarkerSetCsvReader().importCsv(db, tmpUsedMarkerSet, defaults, dbAction, missingValue);
			tmpUsedMarkerSet.delete();
			return count;
		}else{
			tmpUsedMarkerSet.delete();
			return 0;
