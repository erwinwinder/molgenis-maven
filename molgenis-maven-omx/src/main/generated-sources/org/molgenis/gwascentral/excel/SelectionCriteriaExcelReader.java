...
/* File:        org.molgenis.omx/model/SelectionCriteria.java
...
/**
 * Reads SelectionCriteria from Excel file.
 */
public class SelectionCriteriaExcelReader
{
	private static final Logger logger = Logger.getLogger(SelectionCriteriaExcelReader.class);
			
	/**
	 * Imports SelectionCriteria from a workbook sheet.
	 */
	...
	{
		File tmpSelectionCriteria = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpSelectionCriteria.txt");
		if(tmpSelectionCriteria.exists()){
			boolean deleteSuccess = tmpSelectionCriteria.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpSelectionCriteria.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpSelectionCriteria.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpSelectionCriteria.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpSelectionCriteria);
		if(fileHasHeaders){
			int count = new SelectionCriteriaCsvReader().importCsv(db, tmpSelectionCriteria, defaults, dbAction, missingValue);
			tmpSelectionCriteria.delete();
			return count;
		}else{
			tmpSelectionCriteria.delete();
			return 0;
