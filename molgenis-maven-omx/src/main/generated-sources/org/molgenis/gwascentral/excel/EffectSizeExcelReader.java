...
/**
 * Reads EffectSize from Excel file.
 */
public class EffectSizeExcelReader
{
	private static final Logger logger = Logger.getLogger(EffectSizeExcelReader.class);
			
	/**
	 * Imports EffectSize from a workbook sheet.
	 */
	...
	{
		File tmpEffectSize = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpEffectSize.txt");
		if(tmpEffectSize.exists()){
			boolean deleteSuccess = tmpEffectSize.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpEffectSize.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpEffectSize.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpEffectSize.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpEffectSize);
		if(fileHasHeaders){
			int count = new EffectSizeCsvReader().importCsv(db, tmpEffectSize, defaults, dbAction, missingValue);
			tmpEffectSize.delete();
			return count;
		}else{
			tmpEffectSize.delete();
			return 0;
