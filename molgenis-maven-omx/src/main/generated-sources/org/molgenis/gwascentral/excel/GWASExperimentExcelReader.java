...
/**
 * Reads GWASExperiment from Excel file.
 */
public class GWASExperimentExcelReader
{
	private static final Logger logger = Logger.getLogger(GWASExperimentExcelReader.class);
			
	/**
	 * Imports GWASExperiment from a workbook sheet.
	 */
	...
	{
		File tmpGWASExperiment = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpGWASExperiment.txt");
		if(tmpGWASExperiment.exists()){
			boolean deleteSuccess = tmpGWASExperiment.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpGWASExperiment.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpGWASExperiment.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpGWASExperiment.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpGWASExperiment);
		if(fileHasHeaders){
			int count = new GWASExperimentCsvReader().importCsv(db, tmpGWASExperiment, defaults, dbAction, missingValue);
			tmpGWASExperiment.delete();
			return count;
		}else{
			tmpGWASExperiment.delete();
			return 0;
