...
/* File:        org.molgenis/model/GenotypeFrequency.java
...
/**
 * Reads GenotypeFrequency from Excel file.
 */
public class GenotypeFrequencyExcelReader
{
	private static final Logger logger = Logger.getLogger(GenotypeFrequencyExcelReader.class);
			
	/**
	 * Imports GenotypeFrequency from a workbook sheet.
	 */
	...
	{
		File tmpGenotypeFrequency = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpGenotypeFrequency.txt");
		if(tmpGenotypeFrequency.exists()){
			boolean deleteSuccess = tmpGenotypeFrequency.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpGenotypeFrequency.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpGenotypeFrequency.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpGenotypeFrequency.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpGenotypeFrequency);
		if(fileHasHeaders){
			int count = new GenotypeFrequencyCsvReader().importCsv(db, tmpGenotypeFrequency, defaults, dbAction, missingValue);
			tmpGenotypeFrequency.delete();
			return count;
		}else{
			tmpGenotypeFrequency.delete();
			return 0;
