...
/**
 * Reads PhenotypeProperty from Excel file.
 */
public class PhenotypePropertyExcelReader
{
	private static final Logger logger = Logger.getLogger(PhenotypePropertyExcelReader.class);
			
	/**
	 * Imports PhenotypeProperty from a workbook sheet.
	 */
	...
	{
		File tmpPhenotypeProperty = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpPhenotypeProperty.txt");
		if(tmpPhenotypeProperty.exists()){
			boolean deleteSuccess = tmpPhenotypeProperty.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpPhenotypeProperty.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpPhenotypeProperty.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpPhenotypeProperty.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpPhenotypeProperty);
		if(fileHasHeaders){
			int count = new PhenotypePropertyCsvReader().importCsv(db, tmpPhenotypeProperty, defaults, dbAction, missingValue);
			tmpPhenotypeProperty.delete();
			return count;
		}else{
			tmpPhenotypeProperty.delete();
			return 0;
