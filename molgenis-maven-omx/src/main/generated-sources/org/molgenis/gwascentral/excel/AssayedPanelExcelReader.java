...
/**
 * Reads AssayedPanel from Excel file.
 */
public class AssayedPanelExcelReader
{
	private static final Logger logger = Logger.getLogger(AssayedPanelExcelReader.class);
			
	/**
	 * Imports AssayedPanel from a workbook sheet.
	 */
	...
	{
		File tmpAssayedPanel = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpAssayedPanel.txt");
		if(tmpAssayedPanel.exists()){
			boolean deleteSuccess = tmpAssayedPanel.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpAssayedPanel.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpAssayedPanel.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpAssayedPanel.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpAssayedPanel);
		if(fileHasHeaders){
			int count = new AssayedPanelCsvReader().importCsv(db, tmpAssayedPanel, defaults, dbAction, missingValue);
			tmpAssayedPanel.delete();
			return count;
		}else{
			tmpAssayedPanel.delete();
			return 0;
