...
/**
 * Reads StudyDetails from Excel file.
 */
public class StudyDetailsExcelReader
{
	private static final Logger logger = Logger.getLogger(StudyDetailsExcelReader.class);
			
	/**
	 * Imports StudyDetails from a workbook sheet.
	 */
	...
	{
		File tmpStudyDetails = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpStudyDetails.txt");
		if(tmpStudyDetails.exists()){
			boolean deleteSuccess = tmpStudyDetails.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpStudyDetails.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpStudyDetails.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpStudyDetails.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpStudyDetails);
		if(fileHasHeaders){
			int count = new StudyDetailsCsvReader().importCsv(db, tmpStudyDetails, defaults, dbAction, missingValue);
			tmpStudyDetails.delete();
			return count;
		}else{
			tmpStudyDetails.delete();
			return 0;
