...
/**
 * Reads PhenotypeValue from Excel file.
 */
public class PhenotypeValueExcelReader
{
	private static final Logger logger = Logger.getLogger(PhenotypeValueExcelReader.class);
			
	/**
	 * Imports PhenotypeValue from a workbook sheet.
	 */
	...
	{
		File tmpPhenotypeValue = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpPhenotypeValue.txt");
		if(tmpPhenotypeValue.exists()){
			boolean deleteSuccess = tmpPhenotypeValue.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpPhenotypeValue.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpPhenotypeValue.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpPhenotypeValue.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpPhenotypeValue);
		if(fileHasHeaders){
			int count = new PhenotypeValueCsvReader().importCsv(db, tmpPhenotypeValue, defaults, dbAction, missingValue);
			tmpPhenotypeValue.delete();
			return count;
		}else{
			tmpPhenotypeValue.delete();
			return 0;
