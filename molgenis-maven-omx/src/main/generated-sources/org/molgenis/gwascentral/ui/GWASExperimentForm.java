...
/* File:        Org.molgenis.omx/html/GWASExperiment.java
...
/**
 * A HtmlForm that is preloaded with all inputs for entity GWASExperiment
 * @see EntityForm
 */
public class GWASExperimentForm extends EntityForm<GWASExperiment>
{
	
	public GWASExperimentForm()
	{
		super();
	}
	
	public GWASExperimentForm(GWASExperiment entity)
	{
		super(entity);
	}
	...
	@Override
	public Class<GWASExperiment> getEntityClass()
	{
		return GWASExperiment.class;
	}
	...
	{	
		...
		//Id: Field(entity=GWASExperiment, name=id, type=int, auto=true, nillable=false, readonly=true, default=)
		{
			IntInput input = new IntInput("GWASExperiment_id",getEntity().getId());
		...
		//Identifier: Field(entity=GWASExperiment, name=Identifier, type=string[255], auto=false, nillable=false, readonly=false, default=)
		{
			StringInput input = new StringInput("GWASExperiment_Identifier",getEntity().getIdentifier());
		...
		//Name: Field(entity=GWASExperiment, name=Name, type=string[255], auto=false, nillable=false, readonly=false, default=)
		{
			StringInput input = new StringInput("GWASExperiment_Name",getEntity().getName());
		...
		{
			EnumInput input = new EnumInput("GWASExperiment___Type",getEntity().get__Type());
		...
		//Study: Field(entity=Experiment, name=Study, type=xref[Study.id], xref_label='Identifier', auto=false, nillable=false, readonly=false, default=)
		{
			...
			//XrefInput<GWASExperiment> input = new XrefInput<GWASExperiment>("GWASExperiment_Study", getEntity().getStudy());
			...
			XrefInput<Study> input = new XrefInput<Study>("GWASExperiment_Study", org.molgenis.organization.Study.class, dummy);
		...
		{
			StringInput input = new StringInput("GWASExperiment_Design",getEntity().getDesign());
		...
		{
		    //TODO: when we have JPA this should become:
			//XrefInput<GWASExperiment> input = new XrefInput<GWASExperiment>("GWASExperiment_ExperimentType", getEntity().getExperimentType());
			//create xref dummy object
			...
			XrefInput<OntologyTerm> input = new XrefInput<OntologyTerm>("GWASExperiment_ExperimentType", org.molgenis.observ.target.OntologyTerm.class, dummy);
		...
		//TotalMarkersTested: Field(entity=GWASExperiment, name=TotalMarkersTested, type=int, auto=false, nillable=true, readonly=false, default=)
		{
			IntInput input = new IntInput("GWASExperiment_TotalMarkersTested",getEntity().getTotalMarkersTested());
		...
		//TotalMarkersImported: Field(entity=GWASExperiment, name=TotalMarkersImported, type=int, auto=false, nillable=true, readonly=true, default=)
		{
			IntInput input = new IntInput("GWASExperiment_TotalMarkersImported",getEntity().getTotalMarkersImported());
		...
		{
			TextInput input = new TextInput("GWASExperiment_Objective",getEntity().getObjective());
		...
		{
			TextInput input = new TextInput("GWASExperiment_Outcome",getEntity().getOutcome());
		...
		{
			TextInput input = new TextInput("GWASExperiment_Comments",getEntity().getComments());
		...
		//IndividualDataStatement: Field(entity=GWASExperiment, name=IndividualDataStatement, type=text, auto=false, nillable=true, readonly=false, default=Access to individual-level data must be made to the study authors)
		{
			TextInput input = new TextInput("GWASExperiment_IndividualDataStatement",getEntity().getIndividualDataStatement());
		...
		{
			DatetimeInput input = new DatetimeInput("GWASExperiment_TimeCreated",getEntity().getTimeCreated());
		...
		{
			//TODO: when we have JPA this should become:
			//MrefInput input = new MrefInput("GWASExperiment_AssayedPanels", getEntity().getAssayedPanels());
			...
			MrefInput<Panel> input = new MrefInput<Panel> ("GWASExperiment_AssayedPanels", org.molgenis.observ.target.Panel.class, dummyList);
		...
		{
			//TODO: when we have JPA this should become:
			//MrefInput input = new MrefInput("GWASExperiment_DataSets", getEntity().getDataSets());
			//create xref dummy list of references
			...
			MrefInput<DataSet> input = new MrefInput<DataSet> ("GWASExperiment_DataSets", org.molgenis.observ.DataSet.class, dummyList);
