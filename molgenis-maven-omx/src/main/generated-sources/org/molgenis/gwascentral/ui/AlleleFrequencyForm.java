...
/* File:        Org.molgenis.omx/html/AlleleFrequency.java
...
/**
 * A HtmlForm that is preloaded with all inputs for entity AlleleFrequency
 * @see EntityForm
 */
public class AlleleFrequencyForm extends EntityForm<AlleleFrequency>
{
	
	public AlleleFrequencyForm()
	{
		super();
	}
	
	public AlleleFrequencyForm(AlleleFrequency entity)
	{
		super(entity);
	}
	...
	@Override
	public Class<AlleleFrequency> getEntityClass()
	{
		return AlleleFrequency.class;
	}
	...
	{	
		...
		//Id: Field(entity=AlleleFrequency, name=id, type=int, auto=true, nillable=false, readonly=true, default=)
		{
			IntInput input = new IntInput("AlleleFrequency_id",getEntity().getId());
		...
		{
			StringInput input = new StringInput("AlleleFrequency_Identifier",getEntity().getIdentifier());
		...
		{
			StringInput input = new StringInput("AlleleFrequency_Name",getEntity().getName());
		...
		{
			EnumInput input = new EnumInput("AlleleFrequency___Type",getEntity().get__Type());
		...
		{
			TextInput input = new TextInput("AlleleFrequency_description",getEntity().getDescription());
		...
		//ProtocolUsed: Field(entity=DataSet, name=ProtocolUsed, type=xref[Protocol.id], xref_label='Identifier', auto=false, nillable=true, readonly=false, default=)
		{
			...
			//XrefInput<AlleleFrequency> input = new XrefInput<AlleleFrequency>("AlleleFrequency_ProtocolUsed", getEntity().getProtocolUsed());
			...
			XrefInput<Protocol> input = new XrefInput<Protocol>("AlleleFrequency_ProtocolUsed", org.molgenis.observ.Protocol.class, dummy);
		...
		{
			DatetimeInput input = new DatetimeInput("AlleleFrequency_startTime",getEntity().getStartTime());
		...
		{
			DatetimeInput input = new DatetimeInput("AlleleFrequency_endTime",getEntity().getEndTime());
		...
		//FrequencyCluster: Field(entity=AlleleFrequency, name=FrequencyCluster, type=xref[FrequencyCluster.id], xref_label='Identifier', auto=false, nillable=true, readonly=false, default=)
		{
		    //TODO: when we have JPA this should become:
			//XrefInput<AlleleFrequency> input = new XrefInput<AlleleFrequency>("AlleleFrequency_FrequencyCluster", getEntity().getFrequencyCluster());
			//create xref dummy object
			...
			XrefInput<FrequencyCluster> input = new XrefInput<FrequencyCluster>("AlleleFrequency_FrequencyCluster", org.molgenis.gwascentral.FrequencyCluster.class, dummy);
		...
		//AlleleCombo: Field(entity=AlleleFrequency, name=AlleleCombo, type=text, auto=false, nillable=true, readonly=false, default=)
		{
			TextInput input = new TextInput("AlleleFrequency_AlleleCombo",getEntity().getAlleleCombo());
			
			input.setLabel("Allele combo");
			input.setDescription("Allele combo");
			input.setNillable(true);
			input.setReadonly( isReadonly() || getEntity().isReadonly());
			if(this.getHiddenColumns().contains(input.getName()))
			{	
		...
		//FrequencyAsProportion: Field(entity=AlleleFrequency, name=FrequencyAsProportion, type=decimal, auto=false, nillable=false, readonly=false, default=)
		{
			DecimalInput input = new DecimalInput("AlleleFrequency_FrequencyAsProportion",getEntity().getFrequencyAsProportion());
