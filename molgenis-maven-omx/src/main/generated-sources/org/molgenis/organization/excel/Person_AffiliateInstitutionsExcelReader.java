...
/**
 * Reads Person_AffiliateInstitutions from Excel file.
 */
public class Person_AffiliateInstitutionsExcelReader
{
	private static final Logger logger = Logger.getLogger(Person_AffiliateInstitutionsExcelReader.class);
			
	/**
	 * Imports Person_AffiliateInstitutions from a workbook sheet.
	 */
	...
	{
		File tmpPerson_AffiliateInstitutions = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpPerson_AffiliateInstitutions.txt");
		if(tmpPerson_AffiliateInstitutions.exists()){
			boolean deleteSuccess = tmpPerson_AffiliateInstitutions.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpPerson_AffiliateInstitutions.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpPerson_AffiliateInstitutions.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpPerson_AffiliateInstitutions.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpPerson_AffiliateInstitutions);
		if(fileHasHeaders){
			int count = new Person_AffiliateInstitutionsCsvReader().importCsv(db, tmpPerson_AffiliateInstitutions, defaults, dbAction, missingValue);
			tmpPerson_AffiliateInstitutions.delete();
			return count;
		}else{
			tmpPerson_AffiliateInstitutions.delete();
			return 0;
