...
/* File:        org.molgenis/model/Contribution.java
...
/**
 * Reads Contribution from Excel file.
 */
public class ContributionExcelReader
{
	private static final Logger logger = Logger.getLogger(ContributionExcelReader.class);
			
	/**
	 * Imports Contribution from a workbook sheet.
	 */
	...
	{
		File tmpContribution = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpContribution.txt");
		if(tmpContribution.exists()){
			boolean deleteSuccess = tmpContribution.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpContribution.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpContribution.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpContribution.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpContribution);
		if(fileHasHeaders){
			int count = new ContributionCsvReader().importCsv(db, tmpContribution, defaults, dbAction, missingValue);
			tmpContribution.delete();
			return count;
		}else{
			tmpContribution.delete();
			return 0;
