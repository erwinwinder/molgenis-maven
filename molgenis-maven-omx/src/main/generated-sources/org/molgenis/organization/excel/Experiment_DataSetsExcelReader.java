...
/**
 * Reads Experiment_DataSets from Excel file.
 */
public class Experiment_DataSetsExcelReader
{
	private static final Logger logger = Logger.getLogger(Experiment_DataSetsExcelReader.class);
			
	/**
	 * Imports Experiment_DataSets from a workbook sheet.
	 */
	...
	{
		File tmpExperiment_DataSets = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpExperiment_DataSets.txt");
		if(tmpExperiment_DataSets.exists()){
			boolean deleteSuccess = tmpExperiment_DataSets.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpExperiment_DataSets.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpExperiment_DataSets.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpExperiment_DataSets.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpExperiment_DataSets);
		if(fileHasHeaders){
			int count = new Experiment_DataSetsCsvReader().importCsv(db, tmpExperiment_DataSets, defaults, dbAction, missingValue);
			tmpExperiment_DataSets.delete();
			return count;
		}else{
			tmpExperiment_DataSets.delete();
			return 0;
