...
/**
 * Reads Citation_OntologyTerms from Excel file.
 */
public class Citation_OntologyTermsExcelReader
{
	private static final Logger logger = Logger.getLogger(Citation_OntologyTermsExcelReader.class);
			
	/**
	 * Imports Citation_OntologyTerms from a workbook sheet.
	 */
	...
	{
		File tmpCitation_OntologyTerms = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpCitation_OntologyTerms.txt");
		if(tmpCitation_OntologyTerms.exists()){
			boolean deleteSuccess = tmpCitation_OntologyTerms.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpCitation_OntologyTerms.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpCitation_OntologyTerms.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpCitation_OntologyTerms.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpCitation_OntologyTerms);
		if(fileHasHeaders){
			int count = new Citation_OntologyTermsCsvReader().importCsv(db, tmpCitation_OntologyTerms, defaults, dbAction, missingValue);
			tmpCitation_OntologyTerms.delete();
			return count;
		}else{
			tmpCitation_OntologyTerms.delete();
			return 0;
