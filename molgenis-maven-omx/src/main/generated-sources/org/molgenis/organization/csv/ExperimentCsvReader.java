...
/* File:        org.molgenis.omx/model/Experiment.java
...
/**
 * Reads Experiment from a delimited (csv) file, resolving xrefs to ids where needed, that is the tricky bit ;-)
 */
public class ExperimentCsvReader extends CsvToDatabase<Experiment>
{
	private static final Logger logger = Logger.getLogger(ExperimentCsvReader.class);
	...
	public int importCsv(final Database db, CsvReader reader, final Tuple defaults, final DatabaseAction dbAction, final String missingValues) throws DatabaseException, IOException, Exception 
	{
		...
		List<Experiment> experimentsMissingRefs = new ArrayList<Experiment>();
	
		//cache for objects to be imported from file (in batch)
		final List<Experiment> experimentList = new ArrayList<Experiment>(BATCH_SIZE);
		//wrapper to count
		...
		for(Tuple tuple: reader)
		{
			...
			experimentList.add(object);		
			
			//add to db when batch size is reached
			if(experimentList.size() == BATCH_SIZE)
			{
				//resolve foreign keys and copy those entities that could not be resolved to the missingRefs list
				experimentsMissingRefs.addAll(resolveForeignKeys(db, experimentList));
				experimentList.removeAll(experimentsMissingRefs);
				
				//update objects in the database using xref_label defined secondary key(s) 'Identifier' defined in xref_label
				db.update(experimentList,dbAction, "Identifier");
		...
		{
			total.set(total.get() + experimentList.size());
		...
		List<Experiment> experiments = new ArrayList<Experiment>(experimentsMissingRefs);

		int iterationCount = 0;

		do
		{
			experimentsMissingRefs = resolveForeignKeys(db, experimentsMissingRefs);
			@SuppressWarnings("unchecked")
			List<Experiment> resolvableexperiments = new ArrayList<Experiment>(CollectionUtils.disjunction(experiments,
					experimentsMissingRefs));
			experiments.removeAll(resolvableexperiments);
			
			db.update(resolvableexperiments,dbAction, "Identifier");

			if (iterationCount++ > 100)
			{
				throw new Exception(
						"Import of 'experiment' objects failed: attempting to resolve in-list references,"
								+ "but after 100 iterations there are still experiments referring to Individuals that are neither in the database nor in the list of to-be imported experiments."
								+ "Maybe there is a cyclic reference somewhere ?");
			}
		}
		while (experimentsMissingRefs.size() > 0);

		logger.info("imported " + total.get() + " experiment from CSV");
	 ...
	 */
	private List<Experiment> resolveForeignKeys(Database db, List<Experiment> experimentList) throws Exception
	{
		//keep a list of Experiment instances that miss a reference which might be resolvable later
		List<Experiment> experimentsMissingRefs = new ArrayList<Experiment>();
	
		//resolve xref 'study' from study.Identifier -> study.id
		for(Experiment o: experimentList) 
		{
		...
		for(Experiment o: experimentList) for(String xref_label: o.getAssayedPanels_Identifier())
		{
			if(xref_label != null) 
		...
		for(Experiment o: experimentList) for(String xref_label: o.getDataSets_Identifier())
		{
			if(xref_label != null) 
		...
		for(Experiment o:  experimentList)
		{
			while(true){
				...
				if(o.getStudy_Identifier() != null) 
				{
					...
					{
						throw new Exception("Import of 'Experiment' objects failed: cannot find Study for study_Identifier='"+o.getStudy_Identifier()+"'");
					}
				...
				if(o.getExperimentType_Identifier() != null) 
				{
					...
					{
						throw new Exception("Import of 'Experiment' objects failed: cannot find OntologyTerm for experimentType_Identifier='"+o.getExperimentType_Identifier()+"'");
					}
				...
				if(o.getAssayedPanels_Identifier() != null) 
				{
					...
					boolean breakToNextExperiment = false;
					...
					for(int i = 0; i < listSize; i++)
					{
						...
						if(assayedPanelsKeymap.get(key) == null){
							logger.error("Import of 'Experiment' objects failed: "+o);
							throw new Exception("Import of 'Experiment' objects failed: cannot find assayedPanels_Identifier='"+(o.getAssayedPanels_Identifier() != null && i < o.getAssayedPanels_Identifier().size() ? o.getAssayedPanels_Identifier().get(i) : "null")+"'");
						}
						mrefs.add(assayedPanelsKeymap.get(key));
					}
					if(breakToNextExperiment){
						break;
					}
				...
				if(o.getDataSets_Identifier() != null) 
				{
					...
					for(int i = 0; i < listSize; i++)
					{
						...
						if(dataSetsKeymap.get(key) == null){
							logger.error("Import of 'Experiment' objects failed: "+o);
							throw new Exception("Import of 'Experiment' objects failed: cannot find dataSets_Identifier='"+(o.getDataSets_Identifier() != null && i < o.getDataSets_Identifier().size() ? o.getDataSets_Identifier().get(i) : "null")+"'");
						}
