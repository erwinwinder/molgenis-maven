...
/* File:        Org.molgenis.omx/html/Accession.java
...
/**
 * A HtmlForm that is preloaded with all inputs for entity Accession
 * @see EntityForm
 */
public class AccessionForm extends EntityForm<Accession>
{
	...
	public AccessionForm(Accession entity)
	{
		super(entity);
	}
	...
	@Override
	public Class<Accession> getEntityClass()
	{
	...
	{	
		...
		//Id: Field(entity=Accession, name=id, type=int, auto=true, nillable=false, readonly=true, default=)
		{
			IntInput input = new IntInput("Accession_id",getEntity().getId());
		...
		{
			StringInput input = new StringInput("Accession_Identifier",getEntity().getIdentifier());
		...
		{
			StringInput input = new StringInput("Accession_Name",getEntity().getName());
		...
		{
			EnumInput input = new EnumInput("Accession___Type",getEntity().get__Type());
		...
		//Ontology: Field(entity=OntologyTerm, name=ontology, type=xref[Ontology.id], xref_label='Identifier', auto=false, nillable=true, readonly=false, default=)
		{
			...
			//XrefInput<Accession> input = new XrefInput<Accession>("Accession_ontology", getEntity().getOntology());
			...
			XrefInput<Ontology> input = new XrefInput<Ontology>("Accession_ontology", org.molgenis.observ.target.Ontology.class, dummy);
		...
		{
			StringInput input = new StringInput("Accession_termAccession",getEntity().getTermAccession());
		...
		{
			StringInput input = new StringInput("Accession_definition",getEntity().getDefinition());
