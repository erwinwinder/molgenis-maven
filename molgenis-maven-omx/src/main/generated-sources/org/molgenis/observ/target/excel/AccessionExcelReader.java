...
/**
 * Reads Accession from Excel file.
 */
public class AccessionExcelReader
{
	private static final Logger logger = Logger.getLogger(AccessionExcelReader.class);
			
	/**
	 * Imports Accession from a workbook sheet.
	 */
	...
	{
		File tmpAccession = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpAccession.txt");
		if(tmpAccession.exists()){
			boolean deleteSuccess = tmpAccession.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpAccession.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpAccession.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpAccession.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpAccession);
		if(fileHasHeaders){
			int count = new AccessionCsvReader().importCsv(db, tmpAccession, defaults, dbAction, missingValue);
			tmpAccession.delete();
			return count;
		}else{
			tmpAccession.delete();
