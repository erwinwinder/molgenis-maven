...
/**
 * Reads PanelSource from Excel file.
 */
public class PanelSourceExcelReader
{
	private static final Logger logger = Logger.getLogger(PanelSourceExcelReader.class);
			
	/**
	 * Imports PanelSource from a workbook sheet.
	 */
	...
	{
		File tmpPanelSource = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpPanelSource.txt");
		if(tmpPanelSource.exists()){
			boolean deleteSuccess = tmpPanelSource.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpPanelSource.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpPanelSource.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpPanelSource.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpPanelSource);
		if(fileHasHeaders){
			int count = new PanelSourceCsvReader().importCsv(db, tmpPanelSource, defaults, dbAction, missingValue);
			tmpPanelSource.delete();
			return count;
		}else{
			tmpPanelSource.delete();
			return 0;
