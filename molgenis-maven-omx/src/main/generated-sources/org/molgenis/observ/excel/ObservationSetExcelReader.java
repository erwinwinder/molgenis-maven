...
/**
 * Reads ObservationSet from Excel file.
 */
public class ObservationSetExcelReader
{
	private static final Logger logger = Logger.getLogger(ObservationSetExcelReader.class);
			
	/**
	 * Imports ObservationSet from a workbook sheet.
	 */
	...
	{
		File tmpObservationSet = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpObservationSet.txt");
		if(tmpObservationSet.exists()){
			boolean deleteSuccess = tmpObservationSet.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpObservationSet.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpObservationSet.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpObservationSet.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpObservationSet);
		if(fileHasHeaders){
			int count = new ObservationSetCsvReader().importCsv(db, tmpObservationSet, defaults, dbAction, missingValue);
			tmpObservationSet.delete();
			return count;
		}else{
			tmpObservationSet.delete();
			return 0;
