...
/**
 * Reads Characteristic from Excel file.
 */
public class CharacteristicExcelReader
{
	private static final Logger logger = Logger.getLogger(CharacteristicExcelReader.class);
			
	/**
	 * Imports Characteristic from a workbook sheet.
	 */
	...
	{
		File tmpCharacteristic = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpCharacteristic.txt");
		if(tmpCharacteristic.exists()){
			boolean deleteSuccess = tmpCharacteristic.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpCharacteristic.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpCharacteristic.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpCharacteristic.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpCharacteristic);
		if(fileHasHeaders){
			int count = new CharacteristicCsvReader().importCsv(db, tmpCharacteristic, defaults, dbAction, missingValue);
			tmpCharacteristic.delete();
			return count;
		}else{
			tmpCharacteristic.delete();
			return 0;
