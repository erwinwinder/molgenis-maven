...
/**
 * Reads Genome from Excel file.
 */
public class GenomeExcelReader
{
	private static final Logger logger = Logger.getLogger(GenomeExcelReader.class);
			
	/**
	 * Imports Genome from a workbook sheet.
	 */
	...
	{
		File tmpGenome = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpGenome.txt");
		if(tmpGenome.exists()){
			boolean deleteSuccess = tmpGenome.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpGenome.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpGenome.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpGenome.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpGenome);
		if(fileHasHeaders){
			int count = new GenomeCsvReader().importCsv(db, tmpGenome, defaults, dbAction, missingValue);
			tmpGenome.delete();
			return count;
		}else{
			tmpGenome.delete();
