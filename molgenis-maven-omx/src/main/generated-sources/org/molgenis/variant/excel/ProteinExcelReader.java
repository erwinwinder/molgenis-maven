...
/* File:        org.molgenis/model/Protein.java
...
/**
 * Reads Protein from Excel file.
 */
public class ProteinExcelReader
{
	private static final Logger logger = Logger.getLogger(ProteinExcelReader.class);
			
	/**
	 * Imports Protein from a workbook sheet.
	 */
	...
	{
		File tmpProtein = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpProtein.txt");
		if(tmpProtein.exists()){
			boolean deleteSuccess = tmpProtein.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpProtein.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpProtein.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpProtein.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpProtein);
		if(fileHasHeaders){
			int count = new ProteinCsvReader().importCsv(db, tmpProtein, defaults, dbAction, missingValue);
			tmpProtein.delete();
			return count;
		}else{
			tmpProtein.delete();
