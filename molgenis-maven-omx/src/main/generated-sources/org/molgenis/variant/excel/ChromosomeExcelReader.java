...
/**
 * Reads Chromosome from Excel file.
 */
public class ChromosomeExcelReader
{
	private static final Logger logger = Logger.getLogger(ChromosomeExcelReader.class);
			
	/**
	 * Imports Chromosome from a workbook sheet.
	 */
	...
	{
		File tmpChromosome = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpChromosome.txt");
		if(tmpChromosome.exists()){
			boolean deleteSuccess = tmpChromosome.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpChromosome.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpChromosome.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpChromosome.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpChromosome);
		if(fileHasHeaders){
			int count = new ChromosomeCsvReader().importCsv(db, tmpChromosome, defaults, dbAction, missingValue);
			tmpChromosome.delete();
			return count;
		}else{
			tmpChromosome.delete();
			return 0;
