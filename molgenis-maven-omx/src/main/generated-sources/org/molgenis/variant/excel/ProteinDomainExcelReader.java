...
/**
 * Reads ProteinDomain from Excel file.
 */
public class ProteinDomainExcelReader
{
	private static final Logger logger = Logger.getLogger(ProteinDomainExcelReader.class);
			
	/**
	 * Imports ProteinDomain from a workbook sheet.
	 */
	...
	{
		File tmpProteinDomain = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpProteinDomain.txt");
		if(tmpProteinDomain.exists()){
			boolean deleteSuccess = tmpProteinDomain.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpProteinDomain.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpProteinDomain.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpProteinDomain.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpProteinDomain);
		if(fileHasHeaders){
			int count = new ProteinDomainCsvReader().importCsv(db, tmpProteinDomain, defaults, dbAction, missingValue);
			tmpProteinDomain.delete();
			return count;
		}else{
			tmpProteinDomain.delete();
			return 0;
