...
/* File:        org.molgenis/model/Gene.java
...
/**
 * Reads Gene from Excel file.
 */
public class GeneExcelReader
{
	private static final Logger logger = Logger.getLogger(GeneExcelReader.class);
			
	/**
	 * Imports Gene from a workbook sheet.
	 */
	...
	{
		File tmpGene = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpGene.txt");
		if(tmpGene.exists()){
			boolean deleteSuccess = tmpGene.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpGene.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpGene.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpGene.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpGene);
		if(fileHasHeaders){
			int count = new GeneCsvReader().importCsv(db, tmpGene, defaults, dbAction, missingValue);
			tmpGene.delete();
			return count;
		}else{
			tmpGene.delete();
