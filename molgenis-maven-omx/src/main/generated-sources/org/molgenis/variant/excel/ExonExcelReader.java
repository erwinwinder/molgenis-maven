...
/**
 * Reads Exon from Excel file.
 */
public class ExonExcelReader
{
	private static final Logger logger = Logger.getLogger(ExonExcelReader.class);
			
	/**
	 * Imports Exon from a workbook sheet.
	 */
	...
	{
		File tmpExon = new File(System.getProperty("java.io.tmpdir") + File.separator + "tmpExon.txt");
		if(tmpExon.exists()){
			boolean deleteSuccess = tmpExon.delete();
			if(!deleteSuccess){
				throw new Exception("Deletion of tmp file 'tmpExon.txt' failed, cannot proceed.");
			}
		}
		boolean createSuccess = tmpExon.createNewFile();
		if(!createSuccess){
			throw new Exception("Creation of tmp file 'tmpExon.txt' failed, cannot proceed.");
		}
		boolean fileHasHeaders = writeSheetToFile(sheet, tmpExon);
		if(fileHasHeaders){
			int count = new ExonCsvReader().importCsv(db, tmpExon, defaults, dbAction, missingValue);
			tmpExon.delete();
			return count;
		}else{
			tmpExon.delete();
